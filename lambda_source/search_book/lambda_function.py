...
# 建立 cloudscraper 實例，用於繞過 Cloudflare 防護（指定桌面版瀏覽器）
...
def find_cheapest_book(soup):
    ...
    for book in books:
        ...
        if type_elem and '中文書' in type_elem.text:
            ...
            if price_elem:
                # 第二個 <b> 是實際價格（第一個是折扣）
                b_tags = price_elem.find_all('b')
                price = b_tags[1] if len(b_tags) > 1 else b_tags[0] if b_tags else None
                if price:
    ...
    if cheapest_book_url:
        ...
        # redirect URL 格式: //search.books.com.tw/redirect/move/.../item/{product_id}/...
